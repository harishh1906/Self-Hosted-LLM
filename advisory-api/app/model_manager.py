...
class ModelManager:
    ...
    def get_default_model(self) -> str:
        """Get default model name (with cluster-wide consistency, 60s reload)."""
        with self._lock:
            ...
            if cache_key in self._cache_timestamps:
                ...
                if cache_age < self._cache_ttl:
                    # Cache still valid, return cached value
            ...
            if db_model:
                ...
                logger.debug(f"Reloaded default model from database: {db_model}")
            else:
                # No DB config, use in-memory default but still update timestamp
                self._cache_timestamps[cache_key] = current_time
