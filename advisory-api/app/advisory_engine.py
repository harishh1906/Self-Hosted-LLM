...
from app.prompts import ADVISORY_PROMPT
...
def generate_advisory(
    ...
    shaped_prompt = shape_prompt(ADVISORY_PROMPT, policy)
    
    # 🔹 Build prompt with finding details
    prompt = shaped_prompt.format(
        title=finding.title,
    ...
    model_to_use = None
    primary_model = None
    fallback_model = FALLBACK_MODEL  # Default fallback is always mistral:7b-instruct
    ...
    # Parse LLM output with JSON-first parsing and safety net
    try:
